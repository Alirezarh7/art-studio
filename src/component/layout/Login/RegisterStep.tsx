...
const RegisterStep = ({ phone, onComplete }: RegisterStepProps) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <h2 className="text-lg font-bold text-center text-[var(--primary)]">تکمیل اطلاعات</h2>
      <div className={"grid grid-cols-2 gap-4"}>
        {inputData.name.map((field, index) => (
          <input
            ...
            className="border border-gray-300 rounded-xl p-3 text-sm placeholder:text-card-foreground text-card-foreground"
          />
        ))}
      </div>
      <button onClick={handleRegister} className="bg-[var(--primary)] text-white rounded-xl py-3 hover:opacity-90 transition">
        ثبت‌نام
      </button>
    </div>
  );
